...
module.exports = {
    ...
    async execute(msg, args) {
        if (args[0] === "start") {
            ...
            await msg.guild.channels.cache.get(guildsettings.main.channels.queue).send(msg.client.session.embed);
            msg.client.session.queueId = msg.guild.channels.cache.get(guildsettings.main.channels.queue).lastMessageID;
            console.log(msg.client.session.queueId);
            msg.channel.send(`Started new session
                \n<@&${guildsettings.main.roles.member}> Don't forget to tune in at <#${guildsettings.main.channels.queue}> using the command \`${prefix}join\``);
