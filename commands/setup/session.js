...
module.exports = {
    ...
    async execute(msg, args) {
        if (args[0] === "start") {
            ...
            await msg.guild.channels.cache.get(guildSettings.channels.queue).send(msg.client.session.embed);
            msg.client.session.queueId = msg.guild.channels.cache.get(guildSettings.channels.queue).lastMessageID;
            console.log(msg.client.session.queueId);
            msg.channel.send(`Started new session
                \n<@&${guildSettings.roles.member}> Don't forget to tune in at <#${guildSettings.channels.queue}> using the command \`${prefix}join\``);
