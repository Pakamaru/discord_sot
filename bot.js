...
client.on('voiceStateUpdate', (oldMember, newMember) => {
    ...
    try {
        if(oldUserChannel === null && newUserChannel !== null) {
            ...
            if (newUserChannel.id === guildSettings.dev.channels.waiting_voice){
                ...
                newMember.guild.channels.cache.get(guildSettings.dev.channels.waiting_chat).send(`<@${newMember.member.id}> you successfully joined the queue`);
            }


        } else if(newUserChannel === null){
            // User leaves a voice channel
            if(oldUserChannel.id === guildSettings.dev.channels.waiting_voice)
                newMember.guild.channels.cache.get(guildSettings.dev.channels.logs).send(`<@${newMember.member.id}> left the waiting room channel`);
        }
...
function updateQueue(guild){
    let players = '';
    let counter = 0;
    let queueChannel = guild.channels.cache.get(guildSettings.dev.channels.queue)
